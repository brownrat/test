...
# OpenCV has its own version of Qt that is incompatible with PyQt5. See:
...
# if no image given, print general information
	...
	if (args.triangle):
		...
		for i in range(0, 37):
			...
			xvalues[i] = math.sqrt(3)/2*(l - s)
			yvalues[i] = m - 0.5*(l + s)
		plt.plot(xvalues, yvalues, '-k')
		for i in range(0, 8):
			plt.plot(xvalues[i*5+1], yvalues[i*5+1], 'ok')
			plt.text(xvalues[i*5+1], yvalues[i*5+1], labels[i*5+1])
		ex = (math.sqrt(3)/2)*(el - es) / (el + em + es)
		ey = (em - 0.5*(el + es)) / (el + em + es)
		d65x = (math.sqrt(3)/2)*(d65l - d65s) / (d65l + d65m + d65s)
		d65y = (d65m - 0.5*(d65l + d65s)) / (d65l + d65m + d65s)
		plt.plot(ex, ey, 'ok')
		plt.plot(d65x, d65y, 'ok')
		plt.text(ex, ey, 'E')
		plt.text(d65x, d65y, 'D65')
		
		xborder = np.array([-math.sqrt(3)/2, 0, math.sqrt(3)/2, -math.sqrt(3)/2])
		yborder = np.array([-0.5, 1, -0.5, -0.5])
		...
		# This looks a lot like a wavelength discrimination function, but I'm not sure
		# it has anything to do with the real thing. Try again.
		distmax = 0
		for i in range(340, 700):
			lcur = wl_to_s(i, l1)
			mcur = wl_to_s(i, m1)
			scur = wl_to_s(i, s1)
			lnext = wl_to_s(i+1, l1)
			mnext = wl_to_s(i+1, m1)
			snext = wl_to_s(i+1, s1)
			xcur = math.sqrt(3)/2*(lcur - scur)
			ycur = mcur - 0.5*(lcur + scur)
			xnext = math.sqrt(3)/2*(lnext - snext)
			ynext = mnext - 0.5*(lnext + snext)
		...
		for i in range(340, 700):
			...
			dl = 1
			while True:
				lnext = wl_to_s(i+dl, l1)
				mnext = wl_to_s(i+dl, m1)
				snext = wl_to_s(i+dl, s1)
				xnext = math.sqrt(3)/2*(lnext - snext)
				ynext = mnext - 0.5*(lnext + snext)
				dist = math.dist((xcur, ycur), (xnext, ynext))
				if (dist >= distmax):
	...
	# This doesn't quite work as advertised. I think what I'm having trouble with is
	# how to normalize the values related to the white point/adaptation light.
	# I figured it out -- the maximum value is the maximum value, nothing to do with
	# the adaptation light. Red herring. The adaptation light is supposed to represent
	# half the potential.
	# The white point always ends up slightly off-center even though you'd think it
	...
	if (args.kodak or args.kcheck):
		...
		if (args.kcheck):
			...
			gray20t = np.empty(61)
			for i in range(0, 61):
				gray20t[i] = math.exp(-(nd20[i]))
			gray2005t = np.empty(61)
			for i in range(0, 61):
				gray2005t[i] = math.exp(-(nd20[i] + nd05[i]))
			gray2006t = np.empty(61)
			for i in range(0, 61):
				gray2006t[i] = math.exp(-(nd20[i] + nd06[i]))
			gray2007t = np.empty(61)
			for i in range(0, 61):
				gray2007t[i] = math.exp(-(nd20[i] + nd07[i]))
			gray2008t = np.empty(61)
			for i in range(0, 61):
				gray2008t[i] = math.exp(-(nd20[i] + nd08[i]))
			gray2009t = np.empty(61)
			for i in range(0, 61):
				gray2009t[i] = math.exp(-(nd20[i] + nd09[i]))
			gray2010t = np.empty(61)
			for i in range(0, 61):
				gray2010t[i] = math.exp(-(nd20[i] + nd10[i]))
			gray2020t = np.empty(61)
			for i in range(0, 61):
				gray2020t[i] = math.exp(-(nd20[i] + nd20[i]))
			...
			print("Gray")
			print("+ 2.0")
			gray20 = spectral_rendering(gray20t, start=300)
			print("+ 2.0 + 0.5")
			gray2005 = spectral_rendering(gray2005t, start=300)
			print("+ 2.0 + 0.6")
			gray2006 = spectral_rendering(gray2006t, start=300)
			print("+ 2.0 + 0.7")
			gray2007 = spectral_rendering(gray2007t, start=300)
			print("+ 2.0 + 0.8")
			gray2008 = spectral_rendering(gray2008t, start=300)
			print("+ 2.0 + 0.7")
			gray2009 = spectral_rendering(gray2009t, start=300)
			print("+ 2.0 + 1.0")
			gray2010 = spectral_rendering(gray2010t, start=300)
			print("+ 2.0 + 2.0")
			gray2020 = spectral_rendering(gray2020t, start=300)
		...
		if (args.kodak):
			...
			gray = np.empty(61)
			for i in range(0, 61):
				gray[i] = nd20[i] + nd07[i]
			gray_0 = np.empty(61)
			for i in range(0, 61):
				gray_0[i] = math.exp(-gray[i])
			gray_01 = np.empty(61)
			for i in range(0, 61):
				gray_01[i] = math.exp(-(gray[i] + nd01[i]))
			gray_03 = np.empty(61)
			for i in range(0, 61):
				gray_03[i] = math.exp(-(gray[i] + nd03[i]))
			gray_07 = np.empty(61)
			for i in range(0, 61):
				gray_07[i] = math.exp(-(gray[i] + nd07[i]))
			gray_10 = np.empty(61)
			for i in range(0, 61):
				gray_10[i] = math.exp(-(gray[i] + nd10[i]))
			...
			#print("+ 0.1")
			#red25l[1] = spectral_rendering(red25e_01, start=300)
			print("+ 0.3")
			...
			#print("+ 0.1")
			#yellow15l[1] = spectral_rendering(yellow15e_01, start=300)
			print("+ 0.3")
			...
			#print("+ 0.1")
			#green58l[1] = spectral_rendering(green58e_01, start=300)
			print("+ 0.3")
			...
			#print("+ 0.1")
			#blue47l[1] = spectral_rendering(blue47_01, start=300)
			print("+ 0.3")
			...
			#grayl = np.empty(5)
			#print("Gray (plain white + 2.0 + 0.7)")
			#grayl[0] = spectral_rendering(gray_0, start=300)
			#print("+ 0.1")
			#grayl[1] = spectral_rendering(gray_01, start=300)
			#print("+ 0.3")
			#grayl[2] = spectral_rendering(gray_03, start=300)
			#print("+ 0.7")
			#grayl[3] = spectral_rendering(gray_07, start=300)
			#print("+ 1.0")
			#grayl[4] = spectral_rendering(gray_10, start=300)
